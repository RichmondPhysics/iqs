...
\vspace{0.3cm}
{\par\centering \includegraphics{appendices/instrumentation_fig1.eps} \par}
\vspace{0.3cm}
...
2. Open the {\it DataStudio} application that you will use to perform the lab.

3. With NO force applied to force sensor, press TARE button on side of force sensor.  This sets the sensor to zero.  This is the ONLY time you will press the TARE button.

4. Click ``Setup''.

5. Click ``Calibrate Sensors''.

6. Set 1st calibration point to 0 newtons, press upper ``Read from sensor'' button.

7. HOLDING SENSOR VERTICALLY (with hook down), hang 200g from sensor, set 2nd calibration point to 1.96 newtons, press lower ``Read from sensor'' button.

8. Click ``OK''. Force sensor is now calibrated for the rest of your experiment. Close ``Calibrate Sensors'' window, close ``Setup'' window.

9. While still holding the force sensor still, press ``Start''. Graph should now show a reading of 1.96 N. Press ``Stop''.

10. Try hanging a different mass from the force sensor; press ``Start'' and check that it is reading correctly.
